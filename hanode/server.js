...
function ensureRepoExists() {
    ...
    //   echo "Received push to branch: $branch (old: $oldrev new: $newrev)"

    //   if [ "$branch" = "master" ] || [ "$branch" = "main" ] || [ "$branch" = "release" ]; then
    //     echo "Deploying $branch branch to $GIT_WORK_TREE"
...
app.use('/git', auth, function (req, res) {
    ...
    if (req.url.indexOf('git-upload-pack') > 0 || req.url.indexOf('git-receive-pack') > 0) {
        const cmd = req.url.indexOf('git-upload-pack') > 0 ? 'upload-pack' : 'receive-pack';
            ...
            if (cmd === 'receive-pack' && code === 0) {
                ...
                try {
                    ...
                    if (fs.existsSync(hookPath)) {
                        ...
                        execSync(`cd ${repoPath} && chmod +x hooks/post-receive && ./hooks/post-receive`);
                        console.log('Post-receive hook executed successfully');
                    }
                } catch (err) {
